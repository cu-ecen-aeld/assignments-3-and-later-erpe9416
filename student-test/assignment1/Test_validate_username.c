...
void test_validate_my_username()
{
    ...
    // Verify your /conf/username.txt config file and my_username() functions are setup properly

    // 1)
    const char * hardcoded_username = my_username();
    
    // 2)
    const char * received_username = malloc_username_from_conf_file();
    
    // 3)
    TEST_ASSERT_TRUE_MESSAGE(hardcoded_username, received_username);
}
